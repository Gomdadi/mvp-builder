...
const mockProjectRepo = { findOneOrFail: jest.fn() };
// create(): 인스턴스 생성 / save(): DB INSERT 후 저장된 엔티티 반환
const mockAnalysisDocumentRepo = { count: jest.fn(), create: jest.fn(), save: jest.fn() };

// 테스트용 프로젝트 데이터 — findOneOrFail의 반환값으로 사용
const project = {
  id: 'proj-1',
  name: 'Test App',
...
describe('Phase1Service', () => {
  ...
  describe('run', () => {
    ...
    it('4개 툴 결과를 수집해 DB에 저장하고 doc id를 반환한다', async () => {
      mockProjectRepo.findOneOrFail.mockResolvedValue(project);
      // count가 0 → 기존 문서 없음 → version=1
      mockAnalysisDocumentRepo.count.mockResolvedValue(0);
      // create()는 인스턴스를 반환, save()는 저장된 엔티티를 반환
      mockAnalysisDocumentRepo.create.mockReturnValue({});
      mockAnalysisDocumentRepo.save.mockResolvedValue({ id: 'doc-1' });
      ...
      expect(mockAnalysisDocumentRepo.create).toHaveBeenCalledWith(
    ...
    it('UI_UX_SKILL_PATH가 없으면 designSystem을 null로 저장한다', async () => {
      ...
      expect(mockAnalysisDocumentRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ designSystem: null }),
      );
    });
    ...
    it('userFeedback이 있으면 버전이 증가하고 feedback이 저장된다', async () => {
      ...
      mockAnalysisDocumentRepo.count.mockResolvedValue(1);
      mockAnalysisDocumentRepo.create.mockReturnValue({});
      mockAnalysisDocumentRepo.save.mockResolvedValue({ id: 'doc-2' });
      mockClaudeAgent.runAgentLoop.mockImplementation(simulateAgentLoop);
    ...
    it('4개 툴 중 일부가 호출되지 않으면 에러를 던진다', async () => {
      ...
      expect(mockAnalysisDocumentRepo.save).not.toHaveBeenCalled();
    });
