...
> **목표**: 인증 없는 세션 기반 구조를 유지하며, 프론트엔드 UI, SSE 실시간 상태 연동, E2E 시나리오 테스트, CI/CD 배포 파이프라인을 구축한다.
> **선행 조건**: Sprint 1 완료 (파이프라인 end-to-end 동작 확인)
> **설계 변경 (2026-06-04)**: User 기반 인증을 제거하고 세션(Redis) 기반으로 전환. E2(users 테이블), E3(OAuth/JWT), E4(사용자 관리), E5(프로젝트 CRUD), E6 수정, E9 수정 항목은 아래와 같이 재정의됨.
...
| 에픽 ID | 에픽명 | 설명 | 상태 |
|---------|--------|------|------|
| ~~E2 추가~~ | ~~데이터베이스 추가 마이그레이션~~ | 세션 기반 전환으로 불필요 | ~~취소~~ |
| ~~E3~~ | ~~인증 (GitHub OAuth + JWT)~~ | 인증 제거 결정 | ~~취소~~ |
| ~~E4~~ | ~~사용자 관리 API~~ | 인증 제거 결정 | ~~취소~~ |
| E5 | 프로젝트 CRUD API | 프로젝트 생성/목록/상세 조회 | ⬜ |
| ~~E6 수정~~ | ~~Claude Agent 서비스 수정~~ | Sprint 1에서 세션 기반으로 구현 완료 | ~~완료(S1)~~ |
| ~~E9 수정~~ | ~~GitHub 연동 서비스 수정~~ | Sprint 1에서 세션 기반으로 구현 완료 | ~~완료(S1)~~ |
| ~~E8~~ | ~~SSE 실시간 상태 스트리밍~~ | Sprint 1에서 Redis pub/sub 기반으로 구현 완료 | ~~완료(S1)~~ |
| E10 | 프론트엔드 UI | 화면 구현, 세션 입력 폼, SSE 연동 | ⬜ |
| E11 | 테스트 | E2E 시나리오 테스트 | ⬜ |
...
- **유형**: 개발
- **설명**: 세션 기반 구조로 인증 없음 — 목록 조회 제외 (클라이언트가 localStorage에 projectId 보관)
- **선행 태스크**: 없음
- **완료 기준**:
  - [ ] `POST /v1/projects` — DTO class-validator: name(1~200자), requirements(10~10000자), techStack 필수
  - [ ] `GET /v1/projects/:id` — 프로젝트 상세 조회. 존재하지 않으면 404
  - [ ] `GET /v1/analysis-documents/:id` — Phase 1 완료 후 ERD, API 스펙, 아키텍처 조회. 존재하지 않으면 404
  - [ ] `GET /v1/pipeline-runs/:id/tasks` — Phase 2 완료 후 태스크 목록 조회 (id, name, type, orderIndex, status)
  - [ ] `GET /v1/projects/:id/files` — pipeline_completed 후 S3에서 전체 파일 경로를 트리 구조로 반환
  - [ ] `GET /v1/projects/:id/files?path=...` — 개별 파일 내용 반환 (S3 다운로드)
  - [ ] Unit Test: 각 엔드포인트 정상/404 케이스

#### T-E5-02: SSE 이벤트 페이로드 보강
- **유형**: 개발
- **설명**: phase_completed 이벤트에 조회 API 호출에 필요한 id 추가. task 이벤트에 taskName 추가
- **선행 태스크**: T-E5-01
- **완료 기준**:
  - [ ] `phase_completed PHASE_1` 이벤트에 `analysisDocumentId` 추가
  - [ ] `phase_completed PHASE_2` 이벤트에 `pipelineRunId` 추가
...
- **선행 태스크**: T-E6-04
- **완료 기준**:
  - [x] S3에서 생성 코드 다운로드 → dockerode-compose로 docker-compose.yml 실행 (기술스택 무관)
  - [x] 테스트 실패 시 에러 로그를 Claude에 전달 → 해당 파일 재생성 → 재실행
  - [x] 최대 재시도 횟수(3회) 초과 시 태스크 FAILED 처리
  - [x] 테스트 전체 통과 시 태스크 DONE 처리 후 S3 최종본 확정
  - [x] Unit Test: 재생성 루프 로직 (mock Docker 실행 환경)
