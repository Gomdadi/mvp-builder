...
// Phase1/2/3/4Service가 공통으로 의존하는 ClaudeAgentService, S3Module, DockerModule을 여기서 관리한다.
...
@Module({
  imports: [
    ...
    // Phase4Service → AnalysisDocument Repository
    TypeOrmModule.forFeature([Project, AnalysisDocument, Task]),
    S3Module,     // Phase3/4Service가 생성 코드를 S3에 업로드/다운로드/목록조회
    DockerModule, // Phase4Service가 DockerSandboxService로 종합 sandbox 실행
  ],
  providers: [ClaudeAgentService, Phase1Service, Phase2Service, Phase3Service, Phase4Service],
  // PipelineWorker가 직접 호출하는 서비스만 export — ClaudeAgentService는 내부 의존성이므로 제외
  exports: [Phase1Service, Phase2Service, Phase3Service, Phase4Service],
})
